  ...
  process {
    ...
    foreach($Outter in $OutterArray){
      ...
      foreach($Inner in $InnerArray){
        ...
        $Inner = $Inner  | select *, @{name="MachineType";e={$expression}}
        $Allservers += $Inner
...
build-FTAudit -BaseArray $(Import-Clixml -Path "C:\Amar\ADObjects.xml") -VMwareArray $(Import-Clixml -Path "C:\Amar\vmdata.xml")